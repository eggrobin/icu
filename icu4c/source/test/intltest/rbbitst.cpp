    ...
    RBBIMonkeyKind()
{
    ...
    rules.push_back(std::make_unique<RegexRule>(uR"(Cr [!|\}] × [\#-\&*+0-9=>@-Z\\\^-z~])", uR"([!|\}])", u'×', uR"([\#-\&*+0-9=>@-Z\\\^-z~])"));
    ...
    rules.push_back(std::make_unique<RegexRule>(uR"(Cr ["\#\&*=>~] ÷ [(<\[\{])", uR"(["\#\&*=>~])", u'÷', uR"([(<\[\{])"));
    rules.push_back(std::make_unique<RegexRule>(uR"(Cr [%)+,.:;\\\]] ÷ [<])", uR"([%)+,.:;\\\]])", u'÷', uR"([<])"));
    rules.push_back(std::make_unique<RegexRule>(uR"(Cr [\-] ÷ ["\#\&'*\-<->@-Z\^-z|~])", uR"([\-])", u'÷', uR"(["\#\&'*\-<->@-Z\^-z|~])"));
    rules.push_back(std::make_unique<RegexRule>(uR"(Cr [?] ÷ [\-|])", uR"([?])", u'÷', uR"([\-|])"));
    ...
    rules.push_back(
        std::make_unique<RegexRule>(uR"(Cr PR ÷ OP on Latin-1)", uR"([\\+±¤\$£¥])", u'÷', uR"([¡¿(\[\{])"));
...
void RBBITest::RunMonkey(BreakIterator *bi, RBBIMonkeyKind &mk, const char *name, std::string engineState,
    ...
    while (loopCount < numIterations || numIterations == -1) {
        ...
        if (name == std::string_view("line")) {
            LazyLineBreakIterator::Context<char16_t> context(testText.getBuffer(),
                                                             (unsigned)testText.length(), 0u, 0u);
            for (unsigned i = 0; i < testText.length();) {
                context.Fetch(testText.getBuffer(), (unsigned)testText.length(), i);
                chromiumFastBreaks.push_back(i == 0 ? FastBreakResult::kCanBreak
                                                    : context.ShouldBreakFast(false));
                context.Advance(i);
            }
            chromiumFastBreaks.push_back(FastBreakResult::kCanBreak);
        }
